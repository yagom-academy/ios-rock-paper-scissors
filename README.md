...
벌써부터 굉장히 좋은 고민들을 해주시는 것을 보니 앞으로 정말 빠르게 발전하실 것 같습니다😆
리뷰가 끝이나도 앞으로도 궁금하신점이 있으시면 언제든지 디엠 츄라이츄라이 해주세요😁

코드 반복 사용 문제
Generic의 경우 보내주신 예시 코드에서 printAddValues 메서드가 에러가 나는 이유는 T가 컴파일단에서 어떤 타입으로 올지 컴파일러는 추론할 수 없기 때문에 + 연산이 동작할지를 장담할 수 없어서 에러를 발생시키는 것 같습니다.
protocol을 활용해서 해결할 수 있을 것 같네요. 하지만 아직 조금 이른 단계인 것 같습니다. 나중에 도전해보시는게 좋을 것 같습니다😊

Generic의 사용, tuple, Dictionary 사용 및 class 상속 사용 등 이렇게 문제를 해결하기 위해 다양한 아이디어를 생각해보시고 해결 해보려고 노력하시는 것 너무너무 좋습니다👍
하지만 프로젝트를 진행하면서 아직 완벽하게 이해가 되지 않은 내용이거나 아카데미에서 배우지 않은 부분을 통한 해결 방식이 떠오르실때는 리뷰어에게 바로 질문을 해주시는것이 좋습니다. 완벽하게 이해하지 못한 내용으로 만약 해결을 한다해도 얻어가실 수 있는 것이 적어질 것 같습니다😊

우선 가위바위보 게임과 묵찌빠 게임에서 공통적으로 필요한 기능들이 많다고 생각해주셨는데, 저는 다른 생각입니다. 물론 메서드명, 메서드들이 내포하는 기능이나 프로세스는 분명히 유사하지만, 엄연히 가위바위보와 묵찌빠는 다른 형식이라고 생각합니다.
이처럼 오버로딩을 해야한다는 의미 자체가 다른 형태의 메서드이기 때문에 오버로딩을 한다고 생각합니다😊

그리고 함수처럼 타입 또한 최대한 한가지의 책임만 가지도록 해야한다고 생각합니다. 현재 Game이라는 타입은 크게 봤을 때 가위바위보를 진행하는 책임과 묵찌빠를 진행해야하는 책임 이렇게 두가지 책임을 가지고있습니다. 이 부분을 각각 가위바위보게임 타입과 묵찌빠게임 타입으로 나누어서 가위바위보게임에서 필요할 때 묵찌빠게임을 생성해서 호출해주는 방식으로 진행하면 어떨까요? 이 부분이 잘 이해가 되지 않으시면 꼭 바로바로 질문주세요!😊

README
앞으로도 야곰이 계속해서 말씀해주시겠지만, README 계속해서 꼼꼼하게 작성하시는것을 추천드립니다!! 너무 잘해주셨어요!!👍👍

추가 코멘트

struct 사용
너무 좋습니다! 앞으로 class와 struct의 차이점과 언제 어떤걸 써야하는지에 대한 고민을 계속해서 하실꺼에요! 지금은 모르셔도 되지만, 앞으로 해당 내용에 대해서 조금씩 조금씩 최대한 깊게 파보시면 정말 좋을 것 같습니다😊


## 4. 리뷰어 종합 코멘트에 대한 민쏜, 수꿍 답변
### 1. 코드 반복 사용 문제

마이노, 저희 둘 다 고민이 많았는데 마이노의 피드백을 듣고나니 고민이 많이 해결되었습니다!
저희는 가위바위보와 묵찌빠가 승패 판정 이외에는 거의 비슷하다고 생각했었는데, 오버로딩을 해야한다는 의미 자체가 다른 형태의 메서드이고, 다른 형식이라는 말씀을 듣고 나니 그런 관점에서도 생각해봐야겠다는 생각이 들었습니다.

한편으로, 그래도 하나의 함수를 통해 해결해보고자 노력했었던만큼, 나중에 프로토콜을 배워서 제네릭으로 합쳐보고 싶다는 생각도 듭니다.

### 2. README

리드미 파일을 이렇게 구체적으로 작성한 건 처음이었는데, 완성하고나니 지금까지 배운 것들이 정리도 되고, 뿌듯하네요. 조언 감사합니다!
