...
<a id="5."></a>
## 🖥️ 5. 실행 화면 
☑️ 사용자로부터 메뉴 번호를 입력받아 가위바위보 게임을 실행한다.
...
☑️ 묵찌빠 게임 시 사용자가 잘못 입력한 경우 턴이 컴퓨터에게 넘어가고, 메뉴를 출력한다.

| 묵찌빠 게임 시 잘못 입력한 경우 | 
| :--------: |
| <Img src = "https://i.imgur.com/BGJhtzo.png" width="500"/> |

☑️ 사용자가 종료 메뉴 선택 시 게임이 종료된다.
...
1) 반복문과 재귀함수
    ...
    **🥵 문제점/고민점**
    ...
    처음엔 `While` 반복문을 활용하여 함수를 반복시켰습니다. 함수 반복을 하는 방법으로 반복문만 있는 것이 아니라 재귀함수도 활용하는 방안도 생각해보라는 피드백을 받았습니다. 재귀함수와 반복문 중 어떤 것이 가독성이 좋은지, 효율적일지 고민하였습니다
    
    **🧐 해결방안**
    
    두 가지 모두 시도해본 결과 재귀함수가 가독성이 좀 더 좋다 판단하여 재귀함수를 사용하였습니다.
...
2) 삼항연산자
    ...
    **🥵 문제점/고민점**
    ...
    가위바위보 승리패 조건 비교 시 사용자의 각 3가지의 패에 따라 승리한 경우/ 비긴 경우/ 진 경우 3가지의 경우들에 대한 조건이 필요했습니다. `if`문을 활용하여 이를 비교하고자 하니 비슷한 조건이 반복되게 나와서 이를 좀 더 효율적으로 바꾸고자 고민하였습니다. 삼항연산자를 알게되어 `if`문의 조건을 삼항연산자로 정리하고자 하였습니다.
    
    **🧐 해결방안**
    하지만 삼항연산자를 활용한 코드가 오히려 가독성이 떨어진다는 피드백을 들었습니다. 하여 `if` 구문의 조건을 좀 더 가독성이 좋으면서 효율적으로 바꾸고자 고민하였고, 사용자의 3가지의 패에 따른 구분이 아닌 게임이 이긴 경우/ 진 경우/ 비긴 경우 3가지의 경우를 기준으로 `if`문의 조건을 정리하였습니다.
...
3) 에러 처리 방법 변경
    ...
    **🥵 문제점/고민점**
    ...
    컴퓨터의 패를 만드는 함수에서 에러를 던지기 때문에 패를 비교할 때 `do-catch`를 사용했습니다. 이 때 `do-catch`가 필요한 이유가 이것밖에 없었기 때문에 `do-catch` 없이 만드는 것을 시도해 보았습니다.
...
4) 함수 분리
    ...
    **🥵 문제점/고민점**
    ...
    하나의 함수 안에 하나의 기능만 있어야 하기 때문에 함수 단위에 대한 고민을 많이 했습니다. 처음에 정의한 `computeGame`함수는 컴퓨터의 패를 생성하는 역할과 사용자의 패와 컴퓨터의 패를 비교하는 두가지의 역할이 들어있었습니다. 이 부분이 한 함수 안에 여러가지 기능을 포함하고 있다고 생각되어 함수 분리를 고민했습니다.
    
    **🧐 해결방안**
    
    기존의 `computeGame`함수를 컴퓨터의 패를 계산하는 함수`createRival`과 사용자와 컴퓨터의 패를 비교하는 함수`decideVictory`로 분리했습니다. 
...
5) enum
    ...
    **🥵 문제점/고민점**
    ...
    처음엔 가위바위보의 경우에 대한 `enum`만 정의를 했습니다. 이후 묵찌빠 게임을 진행하면서 다양한 변수들을 두고 `enum` 처리를 할지 말지 고민을 많이 했습니다. 예를 들어 가위바위보와 묵찌빠 게임 모드를 `enum`으로 만들어 이를 각각의 경우로 받아서 `if`문에서 활용을 할지 말지에 대한 고민도 하였습니다.
    
    **🧐 해결방안**
    게임 모드를 `enum`으로 정의하여 함수를 만드는 방법도 효율적이라고 생각하지만, 가독성 부분에서 struct를 나눠서 진행하는 편이 더 좋다고 판단하였습니다. 그래서 2개의 `struct`를 만들어 사용하면서 가위바위보에 대한 `enum`, 묵찌빠에 대한 `enum`으로 정의하여 사용하였습니다.
...
6) 함수, 변수 네이밍
    ...
    **🥵 고민점**
    ...
    함수를 최소단위로 최대한 나누다 보니 함수명을 설정할 일이 많았습니다. `Struct`도 2개로 분리하였기 때문에 이번 프로젝트를 진행하면서 함수, 변수명을 설정할 일이 많았습니다. 특히 가독성이 좋은 네이밍을 찾고싶은 욕심에 더욱 고민을 많이 했습니다. 아직은 자주 쓰는 동사, 명사들에 대한 경험이 부족하여 네이밍에 아쉬움이 많았지만 수정에 수정을 거듭하면서 조금은 네이밍에 익숙해지는 거 같습니다.
